...
public static class SqrdmdSource
{
    ...
    public static float CALC_SUM(float a, float b, float c, float d, float rnd)
    {
    ...
    public static float SAVE_SUM(int a)
    {
    ...
    public static int sqrdmd(long seed, float[] map, int size, float rgh)
    {
        SimpleRandom randsource = new SimpleRandom(Convert.ToUInt32(seed));
        ...
        temp = size - 1;
        // MUST EQUAL TO 2^x + 1!
        // Assert.IsTrue(!(temp & (temp - 1) || temp & 3), "Side should be 2**n +1");
        temp = size;
        slope = rgh;
        step = size & ~1;
        ...
        dy = step * size;
        CALC_SUM(map[0], map[step], map[dy], map[dy + step], randsource.next_float_signed());
        SAVE_SUM(i);
        center_sum = sum;
        ...
        /* Top row. */
        p0 = step >> 1;
        CALC_SUM(map[0], map[step], center_sum, center_sum, randsource.next_float_signed());
        SAVE_SUM(p0);
        /* Left column. */
        p1 = p0 * size;
        CALC_SUM(map[0], map[dy], center_sum, center_sum, randsource.next_float_signed());
        SAVE_SUM(p1);
            ...
            *************************************************************/
            ...
            for (y0 = 0, y1 = dy; y1 < size * size; y0 += dy, y1 += dy)
            {
                for (x0 = 0, x1 = dx; x1 < size; x0 += dx, x1 += dx, i += step)
                {
                    ...
                    sum = sum + slope * randsource.next_float_signed();
                    masked = ((int)map[i]);
...
public class Coord
{
    ...
    {
        Assert.IsTrue(x >= 0 && x < width && y >= 0 && y < height, "Coordinates are not valid");
        return y * width + x;
    }
}
